...
def _build_estimator(config, hidden_units=None, warm_start_from=None):
  ...
  base_model = tf.keras.Model(base_model.inputs, outputs=[base_model.get_layer("conv2_block3_out").output])
  ...
  return tf.keras.estimator.model_to_estimator(keras_model=model,config=config,custom_objects={"parse_png_images": parse_png_images})
...
def _create_train_and_eval_spec(trainer_fn_args, schema):
  ...
  exporter = tf.estimator.FinalExporter('data-centric', serving_receiver_fn)
